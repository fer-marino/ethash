...
namespace ethash {
    ...
    // Verify constants:
    static_assert(sizeof(hash512) == light_cache_item_size, "");
    static_assert(sizeof(hash1024) == full_dataset_item_size, "");
    ...
    namespace generic {
        void build_light_cache(
            ...
            for (int q = 0; q < light_cache_rounds; ++q) {
                for (int i = 0; i < num_items; ++i) {
                    const auto index_limit = static_cast<uint32_t>(num_items);
        ...
        epoch_context_full *create_epoch_context(
            ...
            auto *const light_cache = reinterpret_cast<hash512 *>(alloc_data + context_alloc_size);
            ...
            auto *const context = new(alloc_data) epoch_context_full{
                    epoch_number,
    ...
    struct item_state {
        ...
        inline item_state(const epoch_context &context, int64_t index) noexcept
        ...
        final() const noexcept { return keccak512(le::uint32s(mix)); }
    };
    ...
    namespace {
        ...
        inline hash256 hash_kernel(
            ...
            const auto index_limit = static_cast<uint32_t>(context.full_dataset_num_items);
...
unsigned int ethash_find_largest_prime(unsigned int bound) {
    for (unsigned int n = bound; n > 1; n--)
        if (boost::multiprecision::miller_rabin_test(n, 25)) {
            // Value n is probably prime, see if (n-1)/2 is also prime:
            if (boost::multiprecision::miller_rabin_test((n - 1) / 2, 25))
                return n;
        }
    return 1;
}
