...
int main(int argc, const char* argv[])
{
    ...
    BOOST_LOG_TRIVIAL(info) << std::fixed << std::setprecision(2);

    // clang-format off
    BOOST_LOG_TRIVIAL(info) << "Fakeminer Benchmark\n\nParameters:"
    ...
    BOOST_LOG_TRIVIAL(info) << "Progress:\n"
              << "                  |-----    hashrate    -----|  |-----    bandwidth   -----|\n"
    ...
    for (int block_number = start_block_number; block_number < end_block_number; ++block_number)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << std::setw(7) << e << std::setw(9) << block_number << std::setw(10)
